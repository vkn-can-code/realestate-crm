...
export async function sendEmailOutreach(db, messageId, adminName = "Admin Recruiter") {
  ...
  if (!["ADMIN_APPROVED", "READY_TO_SEND"].includes(message.status)) {
    throw new Error(`Message must be ADMIN_APPROVED before sending (current: ${message.status}).`);
  }
  ...
  if (prospect.status === "DO_NOT_CONTACT" || prospect.status === "REJECTED") {
    throw new Error(`Cannot send message: Prospect status is '${prospect.status}'.`);
  }

  const contact = db.prepare("SELECT * FROM prospect_contacts WHERE prospect_id = ?").get(message.prospect_id);
  if (!contact || !contact.email) {
    throw new Error("No enriched email address found for candidate. Run contact enrichment first.");
  }
  ...
  db.prepare(`
    ...
    VALUES (?, ?, 'EMAIL_SENT', 'admin', ?, ?)
  `).run(
    `EVT-${newUuid().slice(0, 8)}`,
    message.prospect_id,
    adminName,
    JSON.stringify({ messageId, toEmail: contact.email, subject: message.subject })
  );

  return { ok: true, messageId, sentTo: contact.email, sentAt: nowStr };
}
